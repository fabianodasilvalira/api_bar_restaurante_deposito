...
from app.schemas.comanda import ComandaDigital
...
def read_comanda_by_id(
    ...
    db: Session = Depends(deps.get_db),
    current_user: models.Usuario = Depends(deps.get_current_active_user) # Acesso restrito
) -> Any:
    """
...
def solicitar_fechamento_comanda(
    comanda_id: uuid.UUID,
    db: Session = Depends(deps.get_db),
    current_user: models.Usuario = Depends(deps.get_current_active_user) # Garçom ou cliente (se autenticado)
) -> Any:
    """
