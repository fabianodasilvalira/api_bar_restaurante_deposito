...
def create_cliente(
    *,
    ...
    current_user: models.Usuario = Depends(deps.get_current_active_user) # Apenas usuários logados podem criar clientes
) -> Any:
    """
...
def delete_cliente(
    *,
    db: Session = Depends(deps.get_db),
    cliente_id: uuid.UUID,
    current_user: models.Usuario = Depends(deps.get_current_active_superuser) # Apenas superusuários podem deletar clientes
) -> Any:
    """
    Deleta um cliente.
