...
from app.db.models.fiado import StatusFiado # Importar o Enum
...
def create_fiado_registro(
    *, 
    ...
    Este endpoint é chamado quando um pagamento é feito com o método "Fiado"
...
@router.get("/{fiado_id}", response_model=schemas.Fiado)
...
@router.put("/{fiado_id}/pagar", response_model=schemas.Fiado)
def registrar_pagamento_de_fiado(
    *,
    db: Session = Depends(deps.get_db),
    fiado_id: uuid.UUID,
    valor_pago: Decimal, # Poderia ser um schema FiadoPagamentoCreate com mais detalhes
    current_user: models.Usuario = Depends(deps.get_current_active_user)
) -> Any:
    """
...
def update_fiado_registro(
    *,
    db: Session = Depends(deps.get_db),
    fiado_id: uuid.UUID,
    fiado_in: schemas.FiadoUpdate, # Usar FiadoUpdate que não permite pagamento direto por aqui
    current_user: models.Usuario = Depends(deps.get_current_active_user)
) -> Any:
    """
