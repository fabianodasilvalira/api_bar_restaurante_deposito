...
from app.schemas.mesa import MesaComComandaInfo
...
def create_mesa(
    *, 
    ...
    current_user: models.Usuario = Depends(deps.get_current_active_superuser) # Apenas superusuários podem criar mesas
) -> Any:
    """
    Cria uma nova mesa.
...
def get_mesa_qrcode(
    mesa_id: uuid.UUID,
    db: Session = Depends(deps.get_db)
    # current_user: models.Usuario = Depends(deps.get_current_active_user) # Acesso ao QR Code pode ser público ou restrito
) -> Response:
    """
